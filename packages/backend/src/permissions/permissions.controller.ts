...
export class PermissionsController {
    ...
    @Post()
    create(@CurrentUser() requester: UserPayload, @Body() dto: CreatePermissionDto): Promise<IPermission> {
    ...
    @RequireAnyPermission(permissions.MANAGE_PERMISSIONS)
    @Post('assign-role')
    assignToRole(@CurrentUser() requester: UserPayload, @Body() dto: AssignPermissionDto): Promise<IRolePermission> {
    ...
    unassignFromRole(
        ...
        return this.permissionsService.unassignFromRole(roleId, permissionId, requester);
    }
