...
describe('PermissionsAuthorizationService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<PermissionsAuthorizationService>(
  ...
  it('should allow managing catalog if user has MANAGE_PERMISSIONS', () => {
    expect(() =>
      service.validateCanManageCatalog([permissions.MANAGE_PERMISSIONS]),
    ).not.toThrow();
  });

  it('should throw if user lacks MANAGE_PERMISSIONS for catalog management', () => {
    expect(() =>
      service.validateCanManageCatalog([permissions.VIEW_USERS]),
    ).toThrow(PermissionUnauthorizedException);
  });

  it('should allow assigning permissions if user has MANAGE_PERMISSIONS', () => {
    expect(() =>
      service.validateCanAssignPermissions([permissions.MANAGE_PERMISSIONS]),
    ).not.toThrow();
  });

  it('should throw if user lacks MANAGE_PERMISSIONS for assignments', () => {
    expect(() =>
      service.validateCanAssignPermissions([permissions.VIEW_USERS]),
    ).toThrow(PermissionUnauthorizedException);
  });
});
