...
import { RolePermission, InsertRolePermission } from './schemas/role-permission.schema';
...
export class PermissionsRepository {
    ...
    async create(data: InsertPermission): Promise<Permission> {
        const [permission] = await this.db.insert(schema.permissions).values(data).returning();
        return permission;
    }

    async update(id: string, data: Partial<InsertPermission>): Promise<Permission> {
    ...
    async assignToRole(data: InsertRolePermission): Promise<RolePermission> {
        const [mapping] = await this.db.insert(schema.rolePermissions).values(data).returning();
        return mapping;
    }
