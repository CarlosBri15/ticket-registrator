...
export class PermissionsService {
    ...
    async create(dto: CreatePermissionDto, requester: UserPayload): Promise<IPermission> {
        ...
        if (existing) throw new PermissionConflictException(`Permission "${name}" already exists`);
        ...
        this.logger.log(`Permission created: ${permission.name} by user ${requester.id}`);
        return permission;
    }

    async update(id: string, dto: UpdatePermissionDto, requester: UserPayload): Promise<IPermission> {
        this.permissionsAuthService.validateCanManageCatalog(requester.permissions);
        ...
        if (dto.name && dto.name !== permission.name) {
            ...
            if (existing) throw new PermissionConflictException(`Permission "${dto.name}" already exists`);
        }
    ...
    async assignToRole(dto: AssignPermissionDto, requester: UserPayload): Promise<IRolePermission> {
        this.permissionsAuthService.validateCanAssignPermissions(requester.permissions);
        ...
        this.logger.log(`Permission ${dto.permissionId} assigned to role ${dto.roleId} by user ${requester.id}`);
        return mapping;
    }

    async unassignFromRole(roleId: string, permissionId: string, requester: UserPayload) {
        this.permissionsAuthService.validateCanAssignPermissions(requester.permissions);

        await this.permissionsRepository.unassignFromRole(roleId, permissionId);
        this.logger.log(`Permission ${permissionId} unassigned from role ${roleId} by user ${requester.id}`);
    ...
    async seedDefaultPermissions() {
        ...
        this.logger.log(`Seeding process finished for ${permsToSeed.length} permissions (added/updated)`);
        return { message: `Seeding process finished for ${permsToSeed.length} permissions` };
    }
}
