...
export class ReportsRepository {
    ...
    async findWithFilters(filters: {
        ...
        const results = await this.db.select({ report: schema.reports })
            ...
            .where(where)
            .orderBy(...(Array.isArray(orderBy) ? orderBy : [orderBy || desc(schema.reports.createdAt)]))
            .limit(limit || 10)
            .offset(offset || 0);
    ...
    async findOverlapping(userId: string, startDate: Date, endDate: Date): Promise<Report | undefined> {
        return this.db.query.reports.findFirst({
            where: and(
                eq(schema.reports.userId, userId),
                or(
    ...
    async update(id: string, data: Partial<schema.InsertReport>): Promise<Report | undefined> {
