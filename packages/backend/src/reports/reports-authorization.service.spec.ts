...
describe('ReportsAuthorizationService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<ReportsAuthorizationService>(
      ReportsAuthorizationService,
    );
  });
  ...
  describe('getVisibleUser', () => {
    it('should return user if found', async () => {
      ...
      (usersServiceMock.findActiveById as jest.Mock).mockResolvedValue(
        mockUser,
      );
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(service.getVisibleUser('user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('canViewUserReports', () => {
    ...
    it('should allow GLOBAL authority to view any user', async () => {
      ...
      const result = await service.canViewUserReports(
        globalRequester,
        'target-1',
      );
      expect(result).toBe(true);
