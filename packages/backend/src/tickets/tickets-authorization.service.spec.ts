...
describe('TicketsAuthorizationService', () => {
    ...
    beforeEach(async () => {
        ...
        service = module.get<TicketsAuthorizationService>(TicketsAuthorizationService);
    });
    ...
    describe('validateCanViewReport', () => {
        ...
        it('should delegate to reportsAuthService if not the owner', async () => {
            ...
            const result = await service.validateCanViewReport(requester, otherReport);
            expect(result).toBe(true);
            expect(reportsAuthMock.canViewUserReports).toHaveBeenCalledWith(requester, 'user-2');
        });
    });
    ...
    describe('validateCanModifyReport', () => {
        ...
        it('should allow if requester is the owner', async () => {
            expect(await service.validateCanModifyReport(requester, { userId: 'user-1' })).toBe(true);
        });

        it('should NOT allow if requester is NOT the owner', async () => {
            expect(await service.validateCanModifyReport(requester, { userId: 'user-2' })).toBe(false);
        });
    });
});
