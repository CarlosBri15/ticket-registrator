...
export class TicketNotFoundException extends TicketException {
  constructor(id?: string) {
    super(
      id ? `Ticket with ID ${id} not found` : 'Ticket not found',
...
export class TicketUnauthorizedException extends TicketException {
  constructor(
    message: string = 'You are not authorized to access this ticket',
  ) {
...
export class TicketStatusConflictException extends TicketException {
  constructor(
    message: string = 'Ticket status or report status does not allow this operation',
  ) {
