...
import { Permission, InsertPermission } from '../permissions/schemas/permission.schema';
import { RolePermission, InsertRolePermission } from '../permissions/schemas/role-permission.schema';
...
export class RolesRepository {
    ...
    async findByNameAndCompany(name: string, companyId: string | null): Promise<Role | undefined> {
        return this.db.query.roles.findFirst({
            where: and(
    ...
    async findAllRolePermissions(companyId: string | null): Promise<RolePermission[]> {
        return this.db.query.rolePermissions.findMany({
            where: companyId ? eq(schema.rolePermissions.companyId, companyId) : isNull(schema.rolePermissions.companyId)
        });
    }
    ...
    async getRolePermissionsByNames(roleNames: string[], companyId: string | null) {
        return this.db.query.roles.findMany({
            where: and(
