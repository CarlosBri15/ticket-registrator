...
describe('UsersService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw UserBadRequestException when role not found', async () => {
      ...
      await expect(service.create(createDto, creator)).rejects.toThrow(UserBadRequestException);
    });

    it('should throw UserConflictException when hierarchy check fails', async () => {
      usersRepositoryMock.transaction.mockResolvedValue({ id: 'r1', hierarchy: 100 });
      usersAuthMock.validateHierarchyAssignment.mockReturnValue(false);
      await expect(service.create(createDto, creator)).rejects.toThrow(UserConflictException);
    });

    it('should throw UserUnauthorizedException when creating admin without permission', async () => {
      usersRepositoryMock.transaction.mockResolvedValue({ id: 'r1', hierarchy: AUTHORITY_LEVELS.COMPANY });
      usersAuthMock.validateHierarchyAssignment.mockReturnValue(true);
      usersAuthMock.validateCanCreateAdmin.mockReturnValue(false);
      await expect(service.create(createDto, creator)).rejects.toThrow(UserUnauthorizedException);
    });
    ...
    it('should handle duplicate email error 23505', async () => {
      ...
      usersAuthMock.validateHierarchyAssignment.mockReturnValue(true);
      usersAuthMock.validateDepartmentAssignment.mockReturnValue(true);
      usersRepositoryMock.create.mockRejectedValue({ code: '23505', detail: 'email' });
      await expect(service.create(createDto, creator)).rejects.toThrow('Email already exists');
    });
    ...
    it('should handle duplicate username error 23505', async () => {
      ...
      usersAuthMock.validateHierarchyAssignment.mockReturnValue(true);
      usersAuthMock.validateDepartmentAssignment.mockReturnValue(true);
      usersRepositoryMock.create.mockRejectedValue({ code: '23505', detail: 'username' });
      await expect(service.create(createDto, creator)).rejects.toThrow('Username already exists');
    });
    ...
    it('should create user successfully superadmin case', async () => {
        const superCreator = { ...creator, roleHierarchy: AUTHORITY_LEVELS.GLOBAL };
        usersRepositoryMock.transaction
          .mockResolvedValueOnce({ id: 'r1', hierarchy: 1 }) // role
          .mockResolvedValueOnce({ id: 'c1' }) // company
          .mockResolvedValueOnce([{ id: 'dept-1' }]); // departments
        ...
        const result = await service.create({ ...createDto, companyId: 'custom-company' }, superCreator);
    ...
    it('should handle creation without departments provided', async () => {
        ...
        const result = await service.create({ ...createDto, departmentIds: undefined }, creator);
        expect(result.id).toBe('user-1');
    });
  });
  ...
  describe('findMe', () => {
    ...
    it('should throw UserNotFoundException if user not found', async () => {
      ...
      await expect(service.findMe('user-1')).rejects.toThrow(UserNotFoundException);
    });
  });
  ...
  describe('findAll', () => {
      it('should call findAll with global filters for SuperAdmin', async () => {
          ...
          expect(usersRepositoryMock.findAll).toHaveBeenCalledWith([expect.any(Object)]);
      });
      ...
      it('should call findAll with department-level filters for Manager', async () => {
          const managerCreator = { ...creator, roleHierarchy: AUTHORITY_LEVELS.DEPARTMENT };
          usersRepositoryMock.findAll.mockResolvedValue([mockUser]);
  ...
  describe('update', () => {
    it('should update user successfully', async () => {
      ...
      const result = await service.update('user-1', { name: 'New Name' }, creator);
      expect(result.id).toBe('user-1');
    });
    ...
    it('should handle role update successfully', async () => {
        usersRepositoryMock.findById.mockResolvedValue(mockUser);
        usersAuthMock.validateCanUpdateUser.mockReturnValue(true);
        usersRepositoryMock.transaction.mockResolvedValue({ id: 'new-r-id', hierarchy: 1 });
        usersRepositoryMock.update.mockResolvedValue(mockUser);

        await service.update('user-1', { roleId: 'Employee' } as any, creator);
        expect(usersRepositoryMock.update).toHaveBeenCalledWith('user-1', expect.objectContaining({ roleId: 'new-r-id' }), undefined);
    });
    ...
    it('should throw UserBadRequestException if role update finds no role', async () => {
        ...
        usersAuthMock.validateCanUpdateUser.mockReturnValue(true);
        usersRepositoryMock.transaction.mockResolvedValue(null);
        await expect(service.update('user-1', { roleId: 'Bad' } as any, creator)).rejects.toThrow(UserBadRequestException);
    });

    it('should throw UserUnauthorizedException if role update assigns higher hierarchy', async () => {
        usersRepositoryMock.findById.mockResolvedValue(mockUser);
        usersAuthMock.validateCanUpdateUser.mockReturnValue(true);
        usersRepositoryMock.transaction.mockResolvedValue({ id: 'high-r', hierarchy: 999 });
        await expect(service.update('user-1', { roleId: 'Admin' } as any, creator)).rejects.toThrow(UserUnauthorizedException);
    });

    it('should throw UserUnauthorizedException if unauthorized', async () => {
        usersRepositoryMock.findById.mockResolvedValue(mockUser);
        usersAuthMock.validateCanUpdateUser.mockReturnValue(false);
        await expect(service.update('user-1', {}, creator)).rejects.toThrow(UserUnauthorizedException);
    });

    it('should throw UserUnauthorizedException if self-update tries to change forbidden fields', async () => {
        usersRepositoryMock.findById.mockResolvedValue({ ...mockUser, id: 'creator-1' });
        usersAuthMock.validateCanUpdateUser.mockReturnValue(true);
        await expect(service.update('creator-1', { companyId: 'new' } as any, creator)).rejects.toThrow(UserUnauthorizedException);
    });

    it('should handle duplicate key error in update', async () => {
        usersRepositoryMock.findById.mockResolvedValue(mockUser);
        usersAuthMock.validateCanUpdateUser.mockReturnValue(true);
        usersRepositoryMock.update.mockRejectedValue({ code: '23505', detail: 'email' });
        await expect(service.update('user-1', { email: 'dup' }, creator)).rejects.toThrow('Email already exists');
    });

    it('should handle duplicate username error in update', async () => {
        usersRepositoryMock.findById.mockResolvedValue(mockUser);
        usersAuthMock.validateCanUpdateUser.mockReturnValue(true);
        usersRepositoryMock.update.mockRejectedValue({ code: '23505', detail: 'username' });
        await expect(service.update('user-1', { username: 'dup' }, creator)).rejects.toThrow('Username already exists');
    });
    ...
    it('should rethrow generic error in update', async () => {
        ...
        await expect(service.update('user-1', {}, creator)).rejects.toThrow('Fail');
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should throw UserNotFoundException if user not found for deletion', async () => {
        usersRepositoryMock.findById.mockResolvedValue(null);
        await expect(service.remove('u1', creator)).rejects.toThrow(UserNotFoundException);
    });
    ...
    it('should throw UserUnauthorizedException if hierarchy check fails for deletion', async () => {
        ...
        await expect(service.remove('user-1', creator)).rejects.toThrow(UserUnauthorizedException);
    });
  });
