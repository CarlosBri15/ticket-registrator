...
describe('RolesGuard', () => {
    ...
    it('should throw ForbiddenException if user is not present but roles are required', () => {
        ...
        expect(() => guard.canActivate(context)).toThrow('User lacks necessary role');
    });
    ...
    it('should throw ForbiddenException if user does not match required roles', () => {
        jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue([Roles.ADMIN, Roles.SUPERADMIN]);
        const context = mockExecutionContext({
            id: '123',
    ...
    it('should allow access if user has one of several required roles', () => {
        jest.spyOn(reflector, 'getAllAndOverride').mockReturnValue([Roles.MANAGER, Roles.ADMIN]);
        const context = mockExecutionContext({
            id: '123',
            role: Roles.MANAGER,
        });
