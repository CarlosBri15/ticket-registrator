...
import { IOrganization, IOnboardResponse, Roles } from '@ticket-registrator/shared';
...
export class OrganizationService {
  ...
  async onboard(requester: UserPayload, dto: OnboardOrganizationDto): Promise<IOnboardResponse> {
    ...
    if (existing) {
      throw new OrganizationConflictException(`Organization "${company.name}" already exists`);
    }
    ...
    return this.organizationRepository.transaction(async (tx) => {
      ...
      if (!unassignedDept) {
        throw new BadRequestException('System is not ready: global Unassigned department not found.');
      }
      ...
      if (!adminRole) {
        throw new BadRequestException('System is not ready: Admin role not found.');
      }
      ...
      for (const adminInfo of admins) {
        ...
        if (emailExists) {
          throw new OrganizationConflictException(`Email "${adminInfo.email}" is already in use`);
        }

        const tempPassword = randomBytes(8).toString('hex');
        const tempUsername = `admin_${randomBytes(4).toString('hex')}`;
        const hashedPassword = await this.cryptoService.hashPassword(tempPassword);
      ...
      this.logger.log(`Organization onboarded: ${createdCompany.id} by user ${requester.id}`);
  ...
  async update(requester: UserPayload, id: string, dto: UpdateOrganizationDto): Promise<IOrganization> {
    ...
    const updated = await this.organizationRepository.update(id, { orgName: dto.name });
  ...
  async softDelete(requester: UserPayload, id: string): Promise<{ deleted: boolean }> {
    this.organizationAuthService.validateCanDelete(requester);

    const company = await this.organizationRepository.findByIdIncludingDeleted(id);
    if (!company) throw new OrganizationNotFoundException(id);
